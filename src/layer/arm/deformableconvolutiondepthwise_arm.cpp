#include "deformableconvolutiondepthwise_arm.h"
...
//#include "deformableconvolutiondepthwise_5x5.h"

//#include "deformableconvolutiondepthwise_3x3_int8.h"

#if __ARM_NEON
#include "deformableconvolutiondepthwise_3x3_pack4.h"
//#include "deformableconvolutiondepthwise_5x5_pack4.h"
#endif // __ARM_NEON

DEFINE_LAYER_CREATOR(DeformableConvolutionDepthWise_arm)

DeformableConvolutionDepthWise_arm::DeformableConvolutionDepthWise_arm()
{
#if __ARM_NEON
...
int DeformableConvolutionDepthWise_arm::create_pipeline(const Option& opt)
{
    ...
    // tmp_data.create(offset_data_size / 2, 1, (size_t)4, 1, opt.blob_allocator);
    // if (tmp_data.empty())
    //     return -100;
...
int DeformableConvolutionDepthWise_arm::forward(const Mat& bottom_blob, Mat& top_blob, const Option& opt) const
{
    ...
    // static int dfm_index = 0;

    // printf("%d dfmconvdw 0=%d 3=%d 6=%d 7=%d offset_data_size: %d\n", dfm_index++, num_output, stride_w, weight_data_size, group, 2 * kernel_h * kernel_w * outw * outh);

    // Mat offset_data1 = offset_data;
    // offset_data1.create(2 * kernel_h * kernel_w * outw * outh, 1, out_elemsize, 1, opt.blob_allocator);
    ...
    if (channels * elempack == group && group == num_output)
    {
        ...
        if (elempack == 4)
        {
            if (kernel_w == 3 && kernel_h == 3 && dilation_w == 1 && dilation_h == 1 && stride_w == 1 && stride_h == 1)
            {
                ...
                dfmconvdw3x3s1_pack4_neon(bottom_blob_bordered, top_blob, weight_data_pack4, offset_data, tmp_data1, bias_data, opt);
